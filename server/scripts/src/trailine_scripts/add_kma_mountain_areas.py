...
def import_kma_mountain_areas(file_path: str):
    ...
    with get_db() as db:
        for index, row in df.iterrows():
            ...
            existing = db.query(KmaMountainForecastArea).filter_by(code=area.code).first()
            if existing:
...
def main():
    parser = argparse.ArgumentParser(description="KMA Mountain Forecast Areas Excel to DB importer")
