...
from tests.data.course_intervals import POINTS_B_TO_C_DATA, POINTS_A_TO_B_DATA
...
def setup_course_data_no_1():
    ...
    course = CourseFactory.create(
        ...
        links=[
            ...
            {"interval": interval_b_to_c, "position": 2, "is_reversed": True}
        ]
    )
    CourseImageFactory.create(sort_order=1, course_id=course.id)
    CourseImageFactory.create(sort_order=2, course_id=course.id)

    total_length = interval_a_to_b.length_m + interval_b_to_c.length_m
    duration = interval_a_to_b.duration_ab_minutes + interval_b_to_c.duration_ba_minutes
